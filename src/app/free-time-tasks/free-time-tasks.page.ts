...
@Component({
  selector: 'app-free-time-tasks',
  templateUrl: './free-time-tasks.page.html',
  styleUrls: ['./free-time-tasks.page.scss'],
...
export class FreeTimeTasksPage {
  taskList: any[] = [];
  ...
  private async getTasks() {
    ...
    this.taskList = await this.storage.get('free-time-tasks') || [];
  }
  ...
  async openAddTaskModal() {
    ...
    if (role === 'confirm') {
      ...
      this.storage.set('free-time-tasks', this.taskList);
    }
  }
  ...
  async openTimerModal(task: any) {
    ...
    const { data, role } = await modal.onWillDismiss();
    if (role === 'confirm') {
      this.alert.presentAlert('Free Time Task ' + data.name + ' has been completed. Congratulations!!.', 'Success');
